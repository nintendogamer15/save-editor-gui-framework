...
3. Overwrite confirmation, on **every** destination observed to exist, with no suppression. Custom picker implementations may declare that they confirm, and the default is **does not confirm**, but the declaration is now diagnostic only. Revision 3 let it suppress the prompt for the currently-open document as a genuine duplicate; that reasoning depended on `Save As` taking no backup and restating no preservation claim, and step 6 below now makes both false. The OS dialog asks "replace this file?"; the framework's asks "replace this file, having taken a verified backup, with a codec whose preservation claim reads like this", and a picker is not in a position to ask the second (finding F-2 supersedes finding A7). A duplicated prompt costs less than one silent overwrite.
...
6. Backup before **any** destructive replacement — explicit Overwrite + Backup, and equally a `Save As` whose chosen target already exists (finding F-2). Revision 3 attached the backup to the separately-named command and treated `Save As` as safe by construction. That is true only while `Save As` targets a path that does not exist; pointed at an existing file it was strictly more dangerous than the command presented to users as the risky one, which inverted the safety labelling. Only a `Save As` to a genuinely new path writes without a backup, because there is nothing there to lose. The step is **all-or-nothing**: written from the same retained handle that produced the external-change baseline hash, flushed, and its hash compared against that baseline. Any failure at any step aborts the overwrite with the original untouched and the partial backup removed. If the sibling directory is unwritable, the user is offered an explicit alternate location; the workflow never silently proceeds without a backup. The backup filename grammar uses a Windows-safe time separator, carries a random disambiguating component, and is subject to a stated retention cap applied only to files matching the framework's own grammar.
...
9. External-change detection. The baseline hash is captured at decode from the retained handle and re-verified immediately before the replace, not only at the start of the action. A hash is required for a positive result; metadata may serve only as a fast-path negative, since mtime granularity is coarse and trivially restorable. On Windows the original is held with write sharing denied from check through replace, which closes the window against cooperative writers. On Linux **no lock of any kind is taken** — there is no `flock`, `fcntl`, or `F_SETLK` anywhere in the implementation, and earlier revisions describing Linux locks as "advisory" described a mechanism that does not exist (finding F-12). What actually catches an external write there is the SHA-256 re-verification through the retained handle, and `rename` offers no compare-and-swap, so the window is *narrowed* to the last instruction rather than closed. The documentation and the status text say so rather than claiming more.
    **The same asymmetry applies to the temporary file, and is now also narrowed rather than ignored (finding F-7).** The Windows replace re-opens the temporary file, compares its identity, and renames *that handle*, so the object checked and the object renamed are the same object. Linux has no rename-by-descriptor: the identity is re-read with `statx` and `AT_SYMLINK_NOFOLLOW` immediately before `rename(2)` acts on the name, which refuses a swapped temporary but cannot prove none occurred. Previously the Unix path took only paths and made no check at all. Mismatch aborts and prompts; it never auto-overwrites. The baseline updates after a successful write.
10. Permission preservation. `rename(2)` gives the destination the temp file's mode and ownership, which would silently widen a `0600` save to `0644` — the exact opposite of step 5's promise. Before replacing, the framework copies mode, ACL, and extended attributes from the retained original handle onto the temp, aborts if the resulting permission set would be broader than the original, and treats ACL and xattr copying as best-effort with the widening check as the hard gate. Backups inherit the original's mode, not the directory default.
    **The Windows ACL copy did not work at all, and reported that it had (finding F-16).** It read a `FileSecurity` and passed it unmodified to `SetAccessControl`. `FileSystemSecurity.Persist` derives which sections to write from the object's modified flags, all false on a freshly-read descriptor, so `SetSecurityInfo` was never called: nothing was written, nothing threw, and the result was `PermissionCopyStatus.Copied` with a detail string asserting success — on every save. The consequence was not a widening but an unfixable failure: a destination whose own ACL is narrower than its parent directory's inheritable set left the temp holding the broader inherited ACL, and the widening gate then failed that file's own save with `PermissionWidening`. Two further consequences were worse than the surface defect. `PermissionCopyStatus` was not evidence, which is why review did not catch it. And because the no-op always "succeeded", the identity-checked re-open below it — and therefore the control preventing that re-open from becoming an arbitrary-ACL-write primitive — was **unreachable dead code on every save**. The copy now goes through exactly one identity-checked re-open using a share-neutral `FILE_READ_ATTRIBUTES | READ_CONTROL | WRITE_DAC` mask and `SetKernelObjectSecurity`, propagating `SE_DACL_PROTECTED` so a protected original strips the temp's inherited ACEs. Adding `WRITE_DAC` to the shared exclusive-create mask was rejected: it fixes nothing on its own, and it would widen the access request for every caller of the `Io` layer, where it can fail outright on SMB shares and cloud-filter paths. The widening gate also now fails closed when the candidate's permission set cannot be read, instead of reporting that it grants nothing extra.
11. Round-trip verification of unknown data. A codec's preservation capability is **tested, not trusted**: immediately after decode the framework serializes the unmodified document and compares the result against the source. A mismatch means the declaration is empirically false, and the workflow automatically downgrades to the warning-requiring-confirmation path instead of reporting success.
    **A skipped check is reported as skipped (finding F-6).** Both round-trip checks have a documented size limit, and above it the pre-write check simply stopped running while the outcome still read `"Saved."` — "not checked" and "checked and fine" were the same report. The outcome now carries a `RoundTripVerification` verdict and its reason reaches the user-visible message, mirroring what the preservation-claim check already did with `UnknownDataVerification.Skipped`.
...
15. Backup restore (finding F-10). `RestoreFromBackupAsync` puts a backup's bytes back at the open document's path, through the same machinery as any other destructive write: the backup is resolved by the hardened path primitive, decoded with the open document's codec *before* anything is written — a restore that lands bytes the format cannot read leaves the user with neither their edit nor a readable save — the state being replaced is itself backed up so the restore can be undone, the external-change guard runs, and the replace is atomic with the landed bytes verified. The restored document is returned rather than pushed anywhere, because the framework does not own the application's document reference. The UI stays adopter-owned; this exists so that every adopter is not obliged to reimplement it, less correctly, against the same primitives.
16. **An application policy may refuse any write, and the session's save entry points are overridable (finding F-15).** The framework's rules are a floor rather than a ceiling. `SafeFileWorkflowOptions.WritePolicy` is consulted after the destination is settled and before anything destructive happens — on Save As, Overwrite and Restore alike, including a Save As to a brand-new path, so a policy can refuse anything. A policy can only refuse: never loosen a framework rule, never redirect a write, because either would make the guarantees a function of consumer code. A policy that throws refuses, since treating a broken policy as permission would make the strictest thing in the composition root the easiest to defeat. `SaveAsAsync` gained a path-taking overload so the picker is optional, and `DocumentSession<TDocument>` is derivable with virtual save entry points and protected access to the workflow, the open file, the progress sink and the outcome bookkeeping. Previously the picker lived inside `SaveAsAsync` and the session was sealed without exposing its `OpenSaveFile`, so an editor with a stricter policy had to reimplement `IDocumentSession` wholesale or intercept picks through a substituted `IUserInteraction` — policy enforcement smuggled through a dialog service.


17. One operation at a time per open document (finding raised as adopter-owned-by-convention). The change guard and the backup copy both seek the shared retained `FileStream` to zero, so two concurrent operations on one `OpenSaveFile` would interleave those seeks and produce a backup or a baseline covering part of one read and part of another — with nothing detecting it, because every individual step succeeded. Each operation claims the document for its duration and a second one is **refused**, not queued: a save that silently waited its turn would report success for a document state the user never asked to write. Held across the picker too, since two concurrent saves of one document is exactly what this prevents.
...
| A7 | Picker self-asserts that it confirmed; framework suppresses its own prompt | P2 | FIX → **superseded by F-2** | §7 step 3; suppression removed outright |
