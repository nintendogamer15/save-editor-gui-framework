...
**Status: substantively complete, one clause open.** Verification confirmed every criterion that can be evaluated without a CI run, and independently reproduced Linux execution by publishing `linux-x64` self-contained and running it on Ubuntu. Two clauses remain unevaluated because **no push has occurred, so the workflow has never executed**: "builds on Ubuntu" (MSBuild has not run on Linux, leaving case-sensitivity and SDK-acquisition risk untested) and "in the P0 CI workflow". Later phases proceed on P0, since the substance is verified and blocking the entire program on an unpushed workflow would be disproportionate — but this clause closes the moment CI runs, and it must be closed before the §14 release gate.
